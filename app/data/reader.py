...
class NBitParity(Dataset):
    ...
    def __init__(self, nbit=3, nsample_training=5000, nsample_test=1000,  normalized=True, onehot=True):
        ...
        training_x_set = (np.random.rand(nsample_training,nbit) > 0.5).astype(float)
        training_y_set = (np.sum(training_x_set, axis=1) % 2 != 0).astype(float)
        
        test_x_set = (np.random.rand(nsample_test,nbit) > 0.5).astype(float)
        test_y_set = (np.sum(test_x_set, axis=1) % 2 != 0).astype(float)
        
        self.insert(Resource(training_x_set, tofuzzyclass(training_y_set)))
        self.insert(Resource(test_x_set, tofuzzyclass(test_y_set)), 'test')
